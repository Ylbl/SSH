...
AIPage::AIPage(QWidget *parent)
    ...
    if (!music.openFromFile("C:/Users/O/CLionProjects/MyProgram/SSH/include/music/Lovely_Picnic.ogg"))
        qDebug()<<"error";
    /*std::thread t_is_play([&] {
                if (music.getStatus()==sf::SoundSource::Status::Playing&&is_play.load()==false) {
                    qDebug()<<"error1";
                    music.stop();
                }
                if (music.getStatus()==sf::SoundSource::Status::Stopped&&is_play.load()==true) {
                    qDebug()<<"error2";
                    music.play();
                }
            });
    t_is_play.detach();#1#
    detectedObjects = new QStringList();
#pragma region 配置栏
#pragma region 选择框
...
void AIPage::init_ai_cpu() {
    if (!cap.isOpened())
        cap.release();
    std::string projectBasePath = "C:/Users/O/source/repos/Program/SSH/out/build/release/models";
    // Set your ultralytics base path

    bool runOnGPU = false;

    inf_cpu = new Inference(projectBasePath + "/yolo11s.onnx", cv::Size(640, 640), projectBasePath + "classes.txt",
                            runOnGPU);
